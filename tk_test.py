...
def coolpc():
    print('原價屋資料撈取ing......')
    message1_label = tk.Label(work_frame, text='原價屋資料撈取ing......')
    message1_label.pack()
    ...
    message2_label = tk.Label(work_frame, text='原價屋資料撈取完畢!!!')
    message2_label.pack()
    window.update()
...
def sinya():
    ...
    if r.status_code == requests.codes.ok:  # 確認網頁狀態
        ...
        message3_label = tk.Label(work_frame, text='欣亞數位資料撈取ing , 約20秒')
        message3_label.pack()
        window.update()
        ...
        print(text)
        message4_label = tk.Label(work_frame, text=text)
        ...
        message5_label = tk.Label(work_frame, text='欣亞數位資料撈取完畢')
        message5_label.pack()
        window.update()
        print('欣亞數位資料撈取完畢')
        sinya_search()
...
def coolpc_search():
    ...
    for j in coolpcData:
        if all(x in j.lower() for x in keyword) and not any(x in j for x in symbol):
            coolpc_listbox.insert(END, str(j)+' '+str(coolpcData[j]))
...
def sinya_search():
    ...
    for k in sinyaData:
        if all(x in k.lower() for x in keyword):
            sinya_listbox.insert(END, str(k) + ' ' + str(sinyaData[k]))
            # print(k, sinyaData[k])
...
keyword_label = tk.Label(keyword_frame, text='請輸入要搜尋的商品，可以用空格多關鍵字搜尋')
keyword_label.pack()
